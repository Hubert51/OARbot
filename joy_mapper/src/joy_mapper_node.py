...
class JoyMapper(object):
    ...
    def cbJoy(self, joy_msg):
        self.joy = joy_msg
        # if LB button is active, control arm angular velocity
        if self.joy.buttons[4] == 1 and self.joy.buttons[5] == 0:
            self.publishArmAngControl()
            # a = 1
        
        # if RB button is active, control fingers
        elif self.joy.buttons[4] == 0 and self.joy.buttons[5] == 1:
            self.publishFingerControl()
            # a = 1
            
        # if RB and LB buttons are active, control arm linear velocity
        elif self.joy.buttons[4] == 1 and self.joy.buttons[5] == 1:
            self.publishArmLinControl()
            
            
        # If start button is pressed, move the robot to home position
    ...
    def publishControl(self):
        ...
        base_cmd_msg.v_x = self.joy.axes[1] * self.v_gain * 0.5 #  Up/Down Axis stick left
        base_cmd_msg.v_y = self.joy.axes[0] * self.v_gain * 0.5# Left/Right Axis stick left
        base_cmd_msg.omega = self.joy.axes[2] * self.w_gain * 0.5 # Left/Right Axis stick right
    ...
    def publishControlStep(self):
        ...
        if (self.joy.buttons[2] == 1): # b button
            step_cmd_msg.drive = +1 # go up
        elif (self.joy.buttons[1] == 1): # a button
    ...
    def publishArmLinControl(self):
        ...
        pose_vel_msg.twist_linear_x = self.joy.axes[1] * self.v_lin_gain  # Up/Down Axis stick left
        pose_vel_msg.twist_linear_y = self.joy.axes[0] * self.v_lin_gain # Left/Right Axis stick left
        pose_vel_msg.twist_linear_z = self.joy.axes[3] * self.v_lin_gain   # Up/Down Axis stick right
    ...
    def publishArmAngControl(self):
        ...
        #pose_vel_msg.twist_angular_z = self.joy.axes[3] * self.w_ang_gain # Up/Down Axis stick right
        pose_vel_msg.twist_angular_z = self.joy.axes[2] * -self.w_ang_gain # Left/Right Axis stick right
    ...
    def publishFingerControl(self):
        try:
            ...
            positions_temp1[:-1] = [max(0.0, n + self.joy.axes[3]*self.finger_move_gain) for n in self.currentFingerPosition[:-1]]
