function [outputArg1,outputArg2] = close_fridge(hObject,inputArg2)
    % open fridge
    handles = guidata(hObject);

    handles.text2.String = 'Close the Fridge door';
    ...
    init_pos = handles.locations.betweenFridgeOven(1:3);
    init_ori = handles.locations.betweenFridgeOven(4:7);
    robotArm.cartesian_pose_client(init_pos, init_ori, 0);
    
    robotArm.cartesian_pose_client([0.45; 0.; 0.], init_ori, 1);
    
    R = axang2rotm([0 1 0 0.25*pi]);
    quat = rotm2quat(R * quat2rotm(robotArm.getOri()'))';
    robotArm.cartesian_pose_client([0.; -0.0; -0.0], quat, 1);
    
%     pos = handles.locations.endOpenFridge(1:3);
%     ori = handles.locations.endOpenFridge(4:7);
%     robotArm.cartesian_pose_client(pos, ori, 0);
    robotArm.closeFinger([6000.0; 6000.0; 6000.0])
    robotArm.cartesian_pose_client([0.; 0.1; -0.25], quat, 1);

    robotArm.cartesian_pose_client([-0.45; 0.; 0.], quat, 1);
